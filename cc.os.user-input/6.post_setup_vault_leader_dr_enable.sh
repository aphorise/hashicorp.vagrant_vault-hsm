...
else pERR 'VAULT ERROR: Setting "sys/replication/dr/primary/enable"\n' ; fi ;
...
else pERR 'VAULT ERROR: Generating DR Replication "secondory-Token"\n' ; fi ;
...
else pERR 'VAULT ERROR: Unable to write policy "promote"\n' ; fi ;
...
else pERR 'VAULT ERROR: Unable to write  "auth/token/roles/failsafe"\n' ; fi ;
...
else pOUT 'VAULT ERROR: Unable to create dr-token"\n' ; fi ;
